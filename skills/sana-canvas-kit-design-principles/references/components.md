...
Banner components communicate status and errors. They are **not interactive** — users do not
activate a Banner itself. When a response is required, provide separate controls (for example a
Button or link) or use an alert dialog pattern instead.

When Banners appear after form submission, moving focus to the Banner message or to the first error
field can help guide the interaction. When Banners are generated by the system, avoid moving focus
away from the user's current context — use an `aria-live` region to announce the message instead.

#### Keyboard Interaction

Banners are informational, not focusable controls. Do not require Tab, Enter, or Space on the
Banner container itself. Any actions inside a Banner (for example "View details") must be separate,
...
- The Banner is informational — not a button or other interactive control

#### Design Annotations Needed

- Specify the Banner’s error type: Alert or Error
- Specify where focus should move after the Banner appears (for example the first invalid field)
...
- When a Banner is generated from a user action, move focus to the Banner message or the first
  related error field — not to the Banner container as a button.
- When a Banner is generated by the system, avoid moving keyboard focus away from users’ context.
  Use an `aria-live` region to announce the message for screen readers in real time.
...
- Use separate navigation or layout instead of Tabs when content is unrelated or needs simultaneous
  comparison.
- Use Hyperlinks within a paragraph to navigate to another page.
...
Switches must support the following keyboard interactions:
